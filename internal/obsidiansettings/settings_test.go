...
func TestApplyCopiesVimrcToVaultRoot(t *testing.T) {
	root := t.TempDir()
	if err := os.MkdirAll(filepath.Join(root, ".obsidian"), 0o755); err != nil {
		t.Fatal(err)
	}
	source := filepath.Join(root, "source.vimrc")
	if err := os.WriteFile(source, []byte("imap jj <Esc>\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	v, err := vault.Open(root)
	if err != nil {
	...
	if string(got) != "imap jj <Esc>\n" {
		t.Fatalf("got %q", got)
	}
}

func TestPlanMarksUnchangedVimrc(t *testing.T) {
	root := t.TempDir()
	if err := os.MkdirAll(filepath.Join(root, ".obsidian"), 0o755); err != nil {
		t.Fatal(err)
	}
	source := filepath.Join(root, "source.vimrc")
	content := []byte("nmap j gj\n")
	if err := os.WriteFile(source, content, 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".obsidian.vimrc"), content, 0o644); err != nil {
		t.Fatal(err)
	}
	v, err := vault.Open(root)
	if err != nil {
	...
	if cp.Changed {
		t.Fatal("expected unchanged vimrc")
	}
}
